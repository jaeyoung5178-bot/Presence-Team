...
const result=await page.evaluate(async()=>{
  ...
  assert(ring.textContent.includes('CL 리젝 40.0%')&&ring.textContent.includes('SW 리젝 5.0%'),'CL/SW rates missing inside doughnut');
  ...
  assert(ringSlide.includes('CL 리젝 40.0%')&&ringSlide.includes('SW 리젝 5.0%')&&!ringSlide.includes('팀원이 입력'),'PPT doughnut label contract');
...
const output=process.env.PRESENCE_CHART_OUTPUT||'/tmp/presence-recap-design-v46.pptx';
...
for(const width of [1440,1024,390]){
  ...
  await visual.locator('#m-recap').screenshot({path:`/tmp/presence-recap-preview-${width}.png`});
