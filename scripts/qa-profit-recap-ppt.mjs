...
const desktop = await page.evaluate(() => {
  ...
  return {
    ...
    previewChecks: {
      ...
      productivitySlide: preview.includes('한 원 안에서 보는 생산성과 리젝') && preview.includes('실제 인컴') && preview.includes('CL 리젝') && preview.includes('SW 리젝'),
      sharedDenominator: preview.includes('총생산성 100%를') && preview.includes('CL 리젝·SW 리젝') && preview.includes('합계 100%로 나눕니다'),
      oneCompositeRing: document.querySelectorAll('#m-recap .pra-composite-ring').length === 1 && document.querySelectorAll('#m-recap .pra-composite-ring svg').length === 1,
...
if (Object.values(desktop.previewChecks).some((v) => !v) || desktop.chartCount !== 3) failures.push('Preview does not match requested composite-ring structure');
