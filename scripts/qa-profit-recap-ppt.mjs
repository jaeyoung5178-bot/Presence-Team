...
const desktop = await page.evaluate(() => {
  ...
  return {
    ...
    previewChecks: {
      ...
      productivitySlide: preview.includes('Gross Sales의 16.7%가 리젝 손실입니다') && preview.includes('ACTUAL INCOME · 실제 지급 급여') && preview.includes('CLIENT REJECT') && preview.includes('SALESWORKS REJECT'),
      sharedDenominator: preview.includes('Gross Sales Value') && preview.includes('Reject Loss') && preview.includes('원형 조각에 합산하지 않고'),
      bondRemovedFromRing: ![...document.querySelectorAll('#m-recap .pra-slide')].find((el) => el.textContent.includes('팀 생산성 구조'))?.textContent.includes('본드'),
