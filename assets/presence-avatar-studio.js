(function(){
  ...
  /* Release hold: the generated tone variants contained baked rectangular
     recolour seams and recoloured identity landmarks (eyes/beak/comb/feet).
     Only the clean master may ship until a protected body mask or separately
     painted, pixel-audited masters pass the avatar quality gate. */
  var COLORS=[['honey','허니','#f6cf58']];
  /* The detached feather bitmaps overlap the canonical comb/head silhouette.
     Keep the clean canonical feather until fitted masters pass visual QA. */
  var FEATHERS=[['classic','클래식']];
  ...
  function isPublished(item){
    /* Props currently cover too much of the torso or float in front of the
       character. They remain in source control for rework, not production. */
    return !!item&&item.file!=='accessory'&&item.cat!=='prop';
  }
  ...
  function art(p,override,context){
    ...
    html+=imageTag(integrated?'pgp-master':'pgp-base',master||ROOT+'presence-pet-base.png',ROOT+'presence-pet-base.png');
    if(!integrated){html+=(feather!=='classic'?'<img class="pgp-feather" src="'+ROOT+'presence-feather-'+feather+'.png" alt="" draggable="false">':'')+itemLayer(chosen.body,'pgp-body')+itemLayer(chosen.head,'pgp-head');}
  ...
  function shell(){var panel=ensurePanel(),host=panel&&panel.querySelector('.ps-shell');if(!host)return null;var complete=host.querySelector('#asStage')&&host.querySelector('#asGrid')&&host.querySelector('#asColors');if(host.dataset.avatarStudio!=='4'||!complete){host.removeAttribute('data-inventory');host.dataset.avatarStudio='4';host.innerHTML='<div class="avatar-studio-shell"><section class="avatar-stage-card"><div class="as-kicker">MY PRESENCE AVATAR</div><h2 class="as-title" id="asName">나의 삐약이</h2><div class="as-sub">한 마리 · 검수 완료 코디만 공개</div><div class="as-stage" id="asStage"></div><div class="as-summary"><b id="asStageLabel">나의 병아리</b><span id="asOutfitSummary">기본 코디</span></div></section><section class="avatar-wardrobe"><div class="aw-head"><div><h2>Avatar Studio</h2><p>몸색·부리·볏·눈이 겹치거나 사각 경계가 남는 자산은 공개하지 않습니다.<br>독립 원화와 착장 검수를 통과한 코디만 선택할 수 있어요.</p></div><span class="aw-free">QUALITY VERIFIED</span></div><div class="avatar-traits"><div class="trait-row"><b class="trait-label">BODY COLOR</b><div class="trait-options" id="asColors"></div></div><div class="trait-row"><b class="trait-label">FEATHER STYLE</b><div class="trait-options" id="asFeathers"></div></div><p class="trait-quality-note">추가 피부톤·부리·깃 스타일은 보호 마스크와 단일 슬롯 검수를 통과한 뒤 공개됩니다.</p></div><div class="aw-tools"><div class="aw-tabs" id="asTabs"></div><input class="aw-search" id="asSearch" placeholder="아이템 검색" oninput="searchPetInventory(this.value)"></div><div class="aw-grid" id="asGrid"></div></section></div>';}return host;}
